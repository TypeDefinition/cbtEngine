...
NS_CBT_BEGIN
        ...
        static void FlipImageHorizontally(cbtU8* _image, cbtS32 _width, cbtS32 _height, size_t _bytesPerPixel)
        {
            ...
            for (cbtS32 r = 0; r < _height; ++r)
            {
                ...
                for (cbtS32 c = 0; c < (_width >> 1); ++c)
                {
                    ...
                    // Convert it to an unsigned char so that we can iterate byte by byte.
                    memcpy(leftPixel, &row[c * _bytesPerPixel],
                            _bytesPerPixel); // Front Pixel (1st Pixel, 2nd Pixel, 3rd Pixel etc...)
                    memcpy(rightPixel, &row[(_width - c - 1) * _bytesPerPixel],
                            _bytesPerPixel); // Back Pixel (Last Pixel, 2nd Last Pixel, 3rd Last Pixel etc...)
    ...
    public:
        static cbtTexture* Create2DTexture(const cbtStr& _name, const cbtStr& _filePath, cbtBool _flipVertical = true,
                cbtBool _flipHorizontal = false);

        static cbtTexture* CreateCubeMap(const cbtStr& _name, std::array<cbtStr, CBT_CUBEMAP_MAX_SIDES> _filePath,
                cbtBool _flipVertical = false, cbtBool _flipHorizontal = true);
    };
