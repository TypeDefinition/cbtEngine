...
// Vertex Array Object
    ...
    public:
        ...
        inline cbtU32 GetVBOCount() const
        {
            return (cbtU32)m_VBOs.size();
        }

        const cbtVertexBuffer* const* GetVBOs() const
        {
            return m_VBOs.empty() ? nullptr : &m_VBOs[0];
        }

        cbtVertexBuffer** GetVBOs()
        {
            return m_VBOs.empty() ? nullptr : &m_VBOs[0];
        }

        const cbtElementBuffer& GetEBO() const
        {
            return *m_EBO;
        }
