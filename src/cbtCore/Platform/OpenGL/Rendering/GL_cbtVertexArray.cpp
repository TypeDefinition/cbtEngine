...
NS_CBT_BEGIN
    ...
    cbtVertexArray* cbtVertexArray::CreateVAO()
    {
        return new GL_cbtVertexArray();
    }

    GL_cbtVertexArray::GL_cbtVertexArray()
            :m_AttributeCounter(0), m_VBOCounter(0)
    {
    ...
    GL_cbtVertexArray::~GL_cbtVertexArray()
    {
        ...
        for (cbtU32 i = 0; i < m_VBOs.size(); ++i)
        { m_VBOs[i]->AutoRelease(); } // Delete VBOs
        if (m_EBO)
        { m_EBO->AutoRelease(); } // Delete EBO
    }
        ...
        for (cbtU32 i = 0; i < elementCount; ++i)
        {
            ...
            case CBT_F32:
                glVertexArrayAttribFormat(m_VAOName, m_AttributeCounter, elements[i].GetComponentCount(),
                        ToGLDataType(elements[i].GetDataType()),
                        elements[i].IsNormalized() ? GL_TRUE : GL_FALSE, elements[i].GetOffset());
                break;
            case CBT_F64:
