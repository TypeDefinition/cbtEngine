...
public:
    ...
    */
    inline void SetToZero() { std::memset(&m_Value[0], 0, ELEMENT_COUNT * sizeof(T)); }

    /**
        \brief Copy the values of the Matrix to _array in column-major format.
        \param _array The array to copy to.
    */
    inline void CopyToArray(T* _array) const { std::memcpy(&_array[0], &m_Value[0], ELEMENT_COUNT * sizeof(T)); }
